"use client";

import { TodoDto } from "@/app/todo-no-rls/hooks/useTodosController";
import { useState } from "react";
import { AiOutlineDelete } from "react-icons/ai";
import { CiCircleCheck, CiEdit } from "react-icons/ci";

interface TodoListItemProps {
  todo: TodoDto;
  onUpdate: (id: number, content: string) => void;
  onDelete: (id: number) => void;
}

const TodoListItem = ({ todo, onUpdate, onDelete }: TodoListItemProps) => {
  const [isEdit, setIsEdit] = useState(false);
  const [userInput, setUserInput] = useState(todo?.content ?? "");

  const onStartEdit = () => {
    setIsEdit(true);
  };

  // 수정이 끝났을 때
  const onFinishEdit = () => {
    onUpdate(todo.id, userInput);
    setIsEdit(false);
  };

  // 삭제 버튼 클릭 시

  const onClickDelete = () => {
    onDelete(todo.id);
  };

  return (
    <li className="min-h-16 bg-[#B280D9] border border-black rounded-2xl font-bold group">
      <article className="min-h-16 p-4 flex flex-col sm:flex-row gap-4">
        {isEdit ? (
          <input
            className="flex-1 text-lg"
            value={userInput}
            onChange={(e) => {
              setUserInput(e.target.value);
            }}
          ></input>
        ) : (
          <div onClick={onStartEdit} className="flex-1 text-lg cursor-pointer">
            {todo?.content}
          </div>
        )}

        <div className="w-fit hidden group-hover:flex self-end gap-2">
          {isEdit ? (
            <div
              className="h-12 w-12 flex justify-center items-center bg-[#7EBB95] border border-black rounded-2xl cursor-pointer"
              onClick={onFinishEdit}
            >
              <CiCircleCheck size={30} />
            </div>
          ) : (
            <div
              className="h-12 w-12 flex justify-center items-center bg-[#7EBB95] border border-black rounded-2xl cursor-pointer"
              onClick={onStartEdit}
            >
              <CiEdit size={30} />
            </div>
          )}
          <div
            className="h-12 w-12 flex justify-center items-center bg-[#ED7461] border border-black rounded-2xl cursor-pointer"
            onClick={onClickDelete}
          >
            <AiOutlineDelete size={30} />
          </div>
        </div>
      </article>
    </li>
  );
};

export default TodoListItem;
